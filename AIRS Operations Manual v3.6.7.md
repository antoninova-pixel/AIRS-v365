# AIRS OPERATIONS MANUAL v3.6.7
**AI Investment Rating System · Indonesia Stock Exchange (IDX) · Swing horizon 3 days – 6 weeks**
Supersedes v3.6.6 in full. Companion files: *AIRS Session Starter v3.6.6* (boot law summary) and the latest *AIRS Session State* (§3C truth file). On conflict: **file > chat memory; Manual > Starter on procedure; Session State > everything on positions.**
...
**Index Rebalancing / Index Flow Theory (v3.6.7, owner ruling 17-Sep-2026):** standing watch layer inside MACRO.
...
* **Alert rule:** watch the announcement month ahead of the effective date; the trading alpha window runs announcement → rebalance execution (major passive-fund flow clusters around implementation).
* MACRO flags any roster name (holdings / watchlist / shortlist) entering an index-flow window; G6 may credit expected passive demand (announcement-to-execution), never retroactively after implementation.
...
**Barbell-Positioning Rule (v3.6.7):** a stock whose sector is classified as a Loser / unfavorable leg by the current Macro Scenario Barbell analysis is capped at 🟡 WATCH on the Quick Scan — never BUY / MOD BUY — regardless of a score ≥3.8. The score is still computed and shown; the verdict is positionally capped until the barbell reclassifies the sector. Held positions are unaffected (HOLD verdict governed by stops and holdings rulings).
...
**Smart Money / Broker Summary Theory (v3.6.7, owner ruling 17-Sep-2026):**
* **Core principle:** Broker Summary = claim; Price Action + Volume = evidence. Never rely on the broker summary alone.
* **Evidence matrix:** Net Sell + price holds = possible hidden accumulation / silent buying (probe-grade signal, verify). Net Buy + price rises = buying confirmed (full-grade G3 credit). Net Buy + breakout fails = possible distribution / retail trap (G3 capped, flag for contradiction watch).
* **Volume = lie detector:** a broker-summary claim without confirming volume is treated as unverified; volume against the claim downgrades it.
* G3 scoring must state which cell of the evidence matrix the name occupies; unverified claims score the cautious half of the flow band.
...
## §14. Changelog
* **v3.6.7** — Barbell-Positioning Rule (barbell-Loser sectors capped at WATCH on Quick Scan regardless of score ≥3.8; holdings unaffected); Smart Money / Broker Summary Theory integrated into §9 (claim-vs-evidence matrix, volume as lie detector, G3 must state evidence cell); Index Rebalancing / Index Flow Theory added to §2 (MSCI / FTSE GEIS / LQ45 / IDX80 review calendars, announcement→execution alpha window, G6 credit for expected passive demand).
